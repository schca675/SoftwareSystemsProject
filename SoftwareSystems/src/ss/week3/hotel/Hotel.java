...
public class Hotel {
	 ...
	 * (getRoom2().getGuest().getName() == guestName));
	 */
	...
	/*@ pure */ public String toString() {
		String sentence = "The hotel " + getHotelName() + 
				" has 2 rooms namely rooms : room " + getRoom1().getNumber() + "whith guest: ";
		if (room1.getGuest() == null) {
	...
	//@ requires numberNights>0;
	/**
	 * Returns the bill of the guest.
	 * 
	 * @param guestName
	 *            Name of the guest
	 * @param numberNights
	 ...
	 * @param theOutStream
	 *            the PrintStream to print the bill to
	 */
	public java.io.PrintStream getBill(String guestName, int numberNights, 
										java.io.PrintStream theOutStream) {
		Room guestRoom = getRoom(guestName);
		boolean guestInHotel = guestRoom != null;
		boolean roomIsPriced = guestRoom instanceof PricedRoom;

		if (!guestInHotel || !roomIsPriced || numberNights <= 0) {
			return null;
		} else {
			Safe guestSafe = guestRoom.getSafe();
			Bill guestBill = new Bill(theOutStream);

			// Add the safe to the bill if it is activated and priced
			if (guestSafe instanceof PricedSafe && guestSafe.isActive()) {
				guestBill.newItem((PricedSafe) guestSafe);
			}

			// Add the nights spent in the priced room.
			for (int i = 1; i <= numberNights; i++) {
				guestBill.newItem((PricedRoom) guestRoom);
			}
